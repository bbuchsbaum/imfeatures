...
using a wide variety of models available through the Python
\`thingsvision\` library. It handles model loading, data preparation,
feature extraction, and returns features in a standard R format.
...
- impaths:
  ...
  Character vector. A vector of full file paths to the images for which
  features should be extracted. The order of features in the output will
  correspond to the order of paths in this vector. The images can be
  located in different directories. A common root directory is
  automatically derived and the paths are passed to the Python dataset
  relative to that root.

- model_name:

  Character string. The name of the model architecture (e.g.,
  \`"resnet50"\`, \`"clip"\`, \`"dino-vit-base-p16"\`). Must be a
  non-empty string. See the Details section of
...
- source:
  ...
  \`"custom"\`. See the Details section of
  [`tv_get_extractor`](https://bbuchsbaum.github.io/imfeatures/reference/tv_get_extractor.md)
  for available sources.

- module_name:

  Character string. The specific layer or module within the model from
  which to extract activations. If NULL (default), automatically selects
  an appropriate layer based on the model architecture. Use
  ...
  like \`"avgpool"\` or \`"fc"\` (in ResNets), \`"classifier.6"\` (in
  VGG/AlexNet), \`"visual"\` (in CLIP), or intermediate layers like
  \`"features.10"\` (in VGG).
...
- model_parameters:
  ...
  Named list (optional). Additional parameters needed for specific
  models (e.g., \`list(variant = "ViT-B/32")\` for CLIP). See
  [`tv_get_extractor`](https://bbuchsbaum.github.io/imfeatures/reference/tv_get_extractor.md)
  documentation for details. Defaults to NULL.

- flatten_acts:

  Logical. If TRUE, flattens activations from intermediate layers (e.g.,
  convolutional or transformer layers with spatial dimensions) into a 2D
  matrix (n_images x n_features). If FALSE, retains the original
...
- batch_size:
  ...
  Integer. Number of images to process in each batch. Adjust based on
  available GPU memory and model size. Defaults to 32.

- temp_out_dir:

  Character string. Path to a directory where a temporary file listing
  the order of processed images will be written. This is required
...
- output_dir:
  ...
  Character string (optional). If provided, features will be saved
  iteratively to this directory in batches (as \`.npy\` files, since
  \`output_type="ndarray"\` is used internally) instead of being
  returned directly. This is useful for very large datasets or models
  that produce large feature maps, preventing potential out-of-memory
  errors in R. If used, the function returns \`NULL\` invisibly.
  Defaults to NULL (features returned in memory).
...
attempted to be set. Returns \`NULL\` invisibly if \`output_dir\` is
specified.

## Details

**Workflow:** This function performs the following steps internally:

1.  Calls
    [`tv_get_extractor`](https://bbuchsbaum.github.io/imfeatures/reference/tv_get_extractor.md)
    to load the specified model via \`reticulate\`.

2.  Creates a \`thingsvision\` \`ImageDataset\` and \`DataLoader\` to
    handle image loading, preprocessing (using transforms from the
    extractor), and batching. It writes a \`file_names.txt\` in
    \`temp_out_dir\` to preserve order.

3.  Calls
    [`tv_extract`](https://bbuchsbaum.github.io/imfeatures/reference/tv_extract.md)
    to perform the batched feature extraction from the specified
    \`module_name\`.

4.  Converts the extracted features (typically NumPy arrays from Python)
    into an R matrix or array.

5.  Optionally attempts to add image basenames (without extension) as
    rownames.

**Finding Module Names:** The \`module_name\` is critical. To find valid
names for your chosen \`model_name\` and \`source\`:


      extractor <- tv_get_extractor("resnet50", "torchvision")
      show_model(extractor) # Prints the model structure

Look for meaningful layer names in the output (e.g., \`avgpool\`,
\`layer4\`, \`fc\` for ResNet).

**Output Dimensions:** The dimensions of the returned object depend on
the layer (\`module_name\`) and the \`flatten_acts\` parameter:

- If \`flatten_acts = TRUE\`: Returns a 2D matrix (n_images x
  n_features).

- If \`flatten_acts = FALSE\`:

  - For typical final layers (like avgpool, fc, classifier): Often
    already 2D (n_images x n_features).

  - For convolutional layers: Returns a 4D array (n_images x channels x
    height x width).

  - For transformer layers (after token handling): May return a 3D array
    (n_images x n_tokens x embedding_dim) or 2D if only CLS token is
    kept. Check output carefully.

**Low-Memory Extraction (\`output_dir\`):** When \`output_dir\` is
specified, features for each batch (or group of batches, controlled by
\`step_size\` in \`tv_extract\`, which this function doesn't expose
directly but uses a default) are saved as separate \`.npy\` files (e.g.,
\`features_0-32.npy\`, \`features_32-64.npy\`, ...) in the specified
directory. The main R function then returns \`NULL\`. You would need to
load and combine these files manually after the function completes,
e.g., using:
...
with \`thingsvision\` and its dependencies installed. Use
[`imfeatures_config`](https://bbuchsbaum.github.io/imfeatures/reference/imfeatures_config.md)
to set this up and configure \`reticulate\` (e.g.,
\`reticulate::use_condaenv("r-thingsvision")\`) before calling this
function.
...
[`tv_extract`](https://bbuchsbaum.github.io/imfeatures/reference/tv_extract.md),
[`tv_create_dataset`](https://bbuchsbaum.github.io/imfeatures/reference/tv_create_dataset.md),

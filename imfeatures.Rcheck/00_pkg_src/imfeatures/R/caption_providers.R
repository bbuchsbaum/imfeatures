...
.select_chat <- function(
  provider = c("openai", "anthropic", "gemini", "azure_openai", 
  ...
  chat <- switch(provider,
    ...
    azure_openai = ellmer::chat_azure(
      system_prompt = system_prompt,
      model = model,
      api_args = api_args,
      echo = echo
...
get_default_caption_model <- function(provider) {
  provider <- match.arg(provider, c("openai", "anthropic", "gemini", 
...
list_caption_providers <- function() {
  providers <- c("openai", "anthropic", "gemini", "azure_openai", 
                "ollama", "huggingface", "openrouter", "vllm")
